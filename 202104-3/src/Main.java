...
public class Main {
    ...
    public static Message discover(String sender,String receiver,int t_i,int time,int t_max,int t_min,int t_def,String H,IPStatus[] ip_status){
        if (!receiver.equals(H) && !receiver.equals("*")){
            //判定字符串相同用equal()函数
            return null;
        }
        Message message;
        ...
        for (int i = 0;i < ip_status.length;i++){
            if (ip_status[i].occupier == sender){
                ...
                message = new Message(H,sender,"OFR",ip_status[i].ip,ip_status[i].time_expect);
                return message;
            }
        }

        //选取最小的状态为未分配的IP地址，未分配为0
        for (int i = 0;i < ip_status.length;i++){
            if (ip_status[i].ipStatus == 0){
                ip_status[i].ipStatus = 1;
                ip_status[i].occupier = sender;
